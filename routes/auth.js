...
router.get('/register', (req, res) => {
    res.render('register', {
        title: 'Регистрация в Ailurophile',
        isRegister: true
    });
});
...
router.post('/login',
    [
        ...
        check('password').isLength({ min: 2 }).withMessage("Длина пароля должна быть больше 2 символов")
    ],
    (req, res, next) => {
...
router.post('/register',
    [
        check('email').isEmail().withMessage("Введите корректный email"),
        check('password').isLength({ min: 2 }).withMessage("Пароль должен быть больше 2 символов"),
